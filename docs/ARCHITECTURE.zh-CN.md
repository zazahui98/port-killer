...
**布局随窗口一起变宽。** 窗口没有最大尺寸限制，若内容宽度写死成一个值，
最大化后两侧会各空出几百像素。因此内容列与抽屉都按 `md` / `xl` 断点分档，
而不是固定一个上限：

| | 480px 窗口 | 900px | 1400px+ |
| --- | --- | --- | --- |
| 内容列 | 560px | 680px | 760px |
| 端口列表抽屉 | 86% | 720px | 880px |

小于 560px 时内容列受视口限制，所以默认的 480px 窗口行为不变。
上限仍然保留 —— 通栏的输入框和进程卡片比居中列更难读。

列表行也一起适配。`md` 以下每行堆叠三行（端口与状态、进程与 PID、地址与路径）；
`md` 以上后两者并排。否则抽屉变宽只是把文字与右侧终结按钮之间的空白拉长 ——
实测在 880px 的行里空出 676px。并排后最大间隙降到约 300px，行高从约 60px 降到约 39px，
一屏能多显示约一半的行数。
...
所以窗口以 `visible: false` 创建，由前端在 React 绘制完成后显示：
...
**嵌套两层 `requestAnimationFrame` 是刻意的**：第一个在 React 提交 DOM 之后触发，
第二个才是浏览器真正绘制完一帧的时刻。只等一个 rAF，仍可能在窗口显示时内容还没上屏。

### 启动画面

`index.html` 里带一个自包含的启动画面：居中的应用标识 + 应用名，
自带 `<style>` 与 `prefers-color-scheme` 分支。它位于 `#root` 内部，
React 挂载时会清空容器，不需要任何清理代码。

它的作用是让失效模式变得体面。`lib.rs` 在 2 秒后**无条件**显示窗口，
因此前端加载失败也不会留下一个永不出现的窗口 —— 而兜底触发时，
用户看到的是这个启动画面，不是一个空白窗口。提前触发同样无害：
在 React 接管之前，启动画面本来就是该看的东西。

兜底调用 `show()` 时**不先查 `is_visible()`**：加了那个判断，
就等于让安全网依赖于它所要防范的东西。

> **验证提示。** 在 Windows 上无法从进程外部可靠地读取窗口可见性。
> `Process.MainWindowHandle` 与 `MainWindowTitle` 在这里不是可靠的代理：
> 对本应用的窗口，即使它就在屏幕上，读到的句柄标题也是空的；
> 而用带标题栏的窗口（记事本）做对照也不成立，因为本应用的窗口是无边框的。
> 请从应用内部验证，或者直接用眼睛看。
