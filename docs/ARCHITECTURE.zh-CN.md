...
而一个在这段时间里可见的窗口只能显示自己的底色 —— 看上去就是启动时闪一下黑。

所以窗口以 `visible: false` 创建，由前端负责显示：
...
**一个永远不显示的隐藏窗口比闪一下更糟**，所以 `lib.rs` 在 3 秒后无条件显示窗口。
这条路径同时兜住了「前端加载失败」的情况 —— 此时用户看到的是 `index.html` 里的
内联占位，一个像是有意为之的标识，而不是一个空白窗口。占位放在 `#root` 内部，
React 挂载时会清空容器，因此不需要任何清理代码。

用临时探针从**应用内部**轮询 `is_visible()` 实测：

| 配置 | setup 时 `is_visible` | 首次可见 |
| --- | --- | --- |
| `show()` 禁用 | `false` | 5343ms（只有兜底） |
| `show()` 启用 | `true` | 立即，即由前端显示 |

> 给后续验证者：`Process.MainWindowHandle` 对**隐藏**窗口也会返回非零句柄，
> 所以外部进程探针测不出窗口可见性。请从应用内部读 `is_visible()`。
